...
ActionType Input::GetUserAction() const
{
	...
	if (UI.InterfaceMode == MODE_DRAW)	//GUI in the DRAW mode
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_ROTATE: return TO_ROTATE;
	...
	
		{
			if (UI.InterfaceMode == MODE_PLAY)
			{
				...
				if (y >= 0 && y < UI.ToolBarHeight)
				{
					...
					switch (ClickedItemOrder)
					{
					case Match: return To_Match_Figures;
					case Missing: return TO_Play_Missing_Figures;
...
void Input::getTwoSelectedFigures(int& indx1, int& indx2) {}

bool Input::isPointInFigure(int findx, Point& pt)
{
	return true;
}
